...
export const qianqista = {
            ...
            if(this.power == 1)
            {
                this.pdatas(function(pres){
                    if(pres.state == 1)
                    {
                        ...
                        },gg.storage.getToalCoin()/gg.config.totalCoinRate);
                    }
                });
            }
        }
    ...
    getOpenId: function(callback)
    {
        ...
        if(wx)
        {
            ...
            else if(config.isTT()) path = "jscode2sessionzijie";
            wx.login({
                force: config.isTT() ? false : true,
                success: function(res)
                {
                    ...
                    self.sendRequest(path,{gameId:self.gameId,gameSecret:self.secret,jsCode:res.code},function(r){
                        if(r.state == 200)
                        {
                            ...
                            if(!self.openid)
                            {
                                var openid = cc.sys.localStorage.getItem("gungod_openid");
                                if(!openid)
                                {
                                    openid = self.guid();
                                    cc.sys.localStorage.setItem("gungod_openid",openid);
                                }
                                self.openid = openid;
                            }
                ...
                fail: function(){
                    var openid = cc.sys.localStorage.getItem("rope_openid");
                    if(!openid)
                    {
                        openid = self.guid();
                        cc.sys.localStorage.setItem("rope_openid",openid);
                    }
                    self.openid = openid;
                    console.error('openid:', self.openid);
                    if(callback)
