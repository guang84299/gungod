...
var gg = window["gg"];
@ccclass
export default class sgmore extends cc.Component {
    ...
    initUI(){
        if(gg.GAME.skipgame && gg.GAME.skipgame.more && gg.GAME.skipgame.more.length>0)
        {
            this.addItem();
            this.itemClick(Math.floor(Math.random()*gg.GAME.skipgame.more.length));
        }
        else
        {
    ...
    addItem(){
        ...
        if(gg.GAME.skipgame && gg.GAME.skipgame.more && i<gg.GAME.skipgame.more.length)
        {
            var data = gg.GAME.skipgame.more[i];
            ...
            // var huobao = cc.find("huobao",item);
            // huobao.active = data.shiwan_tag != "0" ?  true : false;
            // if(huobao.active) item.color = cc.color(255,255,255);
    ...
    itemClick(gtag){
        cc.log(gtag);
        var data = gg.GAME.skipgame.more[gtag];
        var self = this;
        sdk.skipGame(data,function(r){
            if(!self.closeNode.active){
                self.scheduleOnce(function(){self.closeNode.active = true;},2); 
                if(!sdk.judgeLegal())
                {
            ...
            res.openUI("sgrank",null,null,true);           
        });
    }
